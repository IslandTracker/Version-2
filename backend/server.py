...
async def get_current_admin(current_user: User = Depends(get_current_user)):
    logging.info(f"Admin check for user {current_user.email}, is_admin = {current_user.is_admin}")
    if not current_user.is_admin:
        logging.warning(f"User {current_user.email} attempted to access admin endpoint without admin rights")
