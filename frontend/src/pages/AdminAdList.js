...
const AdminAdList = () => {
  ...
  const [deleteModal, setDeleteModal] = useState({ show: false, adId: null, title: '' });
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await axios.delete(`${API}/admin/ads/${deleteModal.adId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      // Remove the deleted ad from the state
      setAds(ads.filter(ad => ad.id !== deleteModal.adId));
      setDeleteModal({ show: false, adId: null, title: '' });
  ...
  const filteredAds = ads.filter(ad => {
    ...
    if (filter !== 'all') {
      matchesFilter = ad.placement === filter;
    }
    
    return matchesSearch && matchesFilter;
  });

  // Get unique placement types
  const placementTypes = [...new Set(ads.map(ad => ad.placement))].filter(Boolean);
                        ...
                        {loading && filteredAds.length === 0 ? (
                          ...
                          filteredAds.map((ad) => (
                            <tr key={ad.id}>
                                ...
                                <div className="flex items-center">
                                  {ad.image_url ? (
                                    <div className="flex-shrink-0 h-10 w-10">
                                      <img className="h-10 w-10 rounded-md object-cover" src={ad.image_url} alt="" />
                                    </div>
                                  ) : (
                                    <div className="flex-shrink-0 h-10 w-10 bg-purple-200 rounded-md flex items-center justify-center">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm text-gray-900">{ad.placement || 'Not specified'}</div>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div className="flex space-x-2 justify-end">
                                  <Link to={`/admin/ads/edit/${ad.id}`} className="text-blue-600 hover:text-blue-900">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                                      <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                    </svg>
                                  </Link>
                                  <button 
                                    onClick={() => setDeleteModal({ show: true, adId: ad.id, title: ad.title || 'this advertisement' })}
          ...
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 className="text-lg leading-6 font-medium text-gray-900">Delete Advertisement</h3>
                    <div className="mt-2">
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                ...
                <button 
                  type="button" 
                  onClick={() => setDeleteModal({ show: false, adId: null, title: '' })}
